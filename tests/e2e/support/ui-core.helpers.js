...
export function requireLoginEnv(test) {
  test.skip(
    ...
    'Defina E2E_LOGIN_EMAIL e E2E_LOGIN_PASSWORD para executar o fluxo real.'
  );
}
...
export async function performLogin(page) {
  ...
  await expect
    .poll(async () => page.evaluate(() => document.body.dataset.authFlow || ''))
    .toBe('authenticated');
  await expect
    .poll(async () => page.evaluate(() => document.body.dataset.authGate || ''))
    .toBe('authenticated');
  await expect
    .poll(async () => page.evaluate(() => document.body.dataset.setupState || ''))
...
export async function selectFirstFilialAndEnter(page, test) {
  ...
  if (!count) {
    test.skip(
      true,
      'Ambiente sem filiais para automacao do setup. Criacao da primeira filial permanece manual no inicio da Onda B.'
    );
  }
  ...
  await expect
    .poll(async () => page.evaluate(() => document.body.dataset.bootstrapState || ''))
    .not.toBe('starting');
}
...
export async function expectBootstrapOk(page) {
  await expect
    ...
    .toBe('ready');
  await expect
    .poll(async () => page.evaluate(() => document.body.dataset.runtimeBootstrap || ''))
    .toBe('ready');
  await expect
    .poll(async () => page.evaluate(() => document.body.dataset.runtimeLoading || ''))
    .toBe('false');
  await expect
    .poll(async () => page.evaluate(() => document.body.dataset.currentScreen || ''))
    .toBe('screen-app');
  await expect
    ...
    .toBe('dashboard');
  await expect
    .poll(async () => page.evaluate(() => document.body.dataset.navState || ''))
    .toBe('ready');
