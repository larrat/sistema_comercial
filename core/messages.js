...
export const MSG = {
  ...
  campanhas: {
    ...
    queueFetchFailed: (err) => guidedMessage({
      severity: SEVERITY.ERROR,
      what: `falha ao buscar clientes elegíveis (${oneLine(err || 'erro desconhecido')})`,
    ...
    queueResult: ({ criados = 0, ignorados = 0, falhas = 0 } = {}) => guidedMessage({
      ...
      next: falhas > 0 ? 'abra o histórico/fila e trate os itens com falha' : 'abra a fila e execute os envios'
    }),
  ...
  jogos: {
    ...
    missingApiUrl: guidedMessage({
      ...
      next: 'informe uma URL válida e sincronize novamente'
    }),
    fetchFailed: (err) => guidedMessage({
      ...
      next: 'valide a URL e conexão; depois tente sincronizar novamente'
    }),
    ...
    syncResult: ({ processados = 0, falhas = 0 } = {}) => guidedMessage({
      ...
      impact: falhas > 0 ? 'alguns jogos podem estar só no cache local' : 'agenda atualizada no banco',
    ...
    invalidForm: guidedMessage({
      ...
      next: 'preencha título e data/hora e tente novamente'
    }),
