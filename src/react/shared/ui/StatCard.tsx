...
export function StatCard({ label, value, description, foot, tone = 'default', onClick }: StatCardProps) {
  ...
  return (
    <section 
      onClick={onClick}
      className={`p-6 rounded-lg border shadow-sm transition-all flex flex-col items-center text-center gap-2 ${toneClasses[tone]} ${onClick ? 'cursor-pointer hover:shadow-md hover:translate-y-[-2px] active:translate-y-0' : ''}`}
    >
