          ...
          {topSegmentos.length > 0 ? (
            <div className="flex flex-col gap-3">
              {topSegmentos.map(([seg, dados]) => (
                <div key={seg} className="p-3 rounded-xl border border-slate-100 hover:border-slate-200 hover:bg-slate-50/50 transition-all flex items-center gap-4">
                  ...
                  <div className="flex-grow min-w-0">
                    <div className="text-sm font-bold text-slate-900 truncate">{seg}</div>
                    <div className="text-[11px] text-slate-500 font-medium">
                      {dados.total} cliente(s) • <span className="text-emerald-600">{dados.marketing} marketing</span>
                    </div>
                  </div>
                </div>
              ))}
