...
export function MetricsGrid({ stats, financeMetrics, periodo }: { stats: any, financeMetrics: any, periodo: string }) {
  const metricCards = [
    ...
    { 
      label: 'Inadimplência', val: financeMetrics?.inadimplencia || 0, prefix: '', suffix: '%', color: (financeMetrics?.inadimplencia || 0) > 5 ? 'text-rose-400' : 'text-emerald-400', borderColor: '#f43f5e', 
      trend: '-', 
      trendLabel: 'Variação N/A', 
      trendUp: (financeMetrics?.inadimplencia || 0) <= 5,
    },
    { 
      label: 'DSO (Prazo)', val: financeMetrics?.dso || 0, prefix: '', suffix: ' dias', color: 'text-indigo-400', borderColor: '#818cf8', 
      trend: '-', 
      trendLabel: 'Variação N/A', 
      trendUp: true,
    }
