...
export function initNotificacoesModule(deps = {}) {
  ...
  renderMetasNegocioSafe =
    typeof deps.renderMetasNegocio === 'function'
      ? deps.renderMetasNegocio
      : renderMetasNegocioSafe;
  registerNotificationKpiSafe =
    typeof deps.registerNotificationKpi === 'function'
      ? deps.registerNotificationKpi
      : registerNotificationKpiSafe;
  logStrategicActionSafe =
    typeof deps.logStrategicAction === 'function'
      ? deps.logStrategicAction
      : logStrategicActionSafe;
}
...
function buildCampanhaNotifications() {
  ...
  const pendenteDias = oldestPending
    ? Math.floor((nowMs - oldestPending.getTime()) / (1000 * 60 * 60 * 24))
    : 0;
...
function buildJogosNotifications(now) {
  ...
  return jogos.flatMap((j) => {
    ...
    if (diff < 0 || diff > 7) return [];
    const mandante = String(j.mandante || '')
      .trim()
      .toLowerCase();
    const visitante = String(j.visitante || '')
      .trim()
      .toLowerCase();
