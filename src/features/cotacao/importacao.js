...
export function renderMapaBody() {
  ...
  const layoutAplicado = ctx.layoutSalvo ? applySavedLayoutToSheet(sheet, ctx.layoutSalvo) : null;

  const gN =
    layoutAplicado && layoutAplicado.idxDescricao >= 0 ? layoutAplicado.idxDescricao : autoN;
  const gC =
    layoutAplicado && layoutAplicado.idxCategoria >= 0 ? layoutAplicado.idxCategoria : autoC;
...
export async function confirmarMapa() {
  ...
  const cIdx =
    document.getElementById('map-cat').value !== ''
      ? parseInt(document.getElementById('map-cat').value, 10)
      : -1;
  const tIdx =
    document.getElementById('map-tabela').value !== ''
      ? parseInt(document.getElementById('map-tabela').value, 10)
      : -1;
  const dIdx =
    document.getElementById('map-desc').value !== ''
      ? parseInt(document.getElementById('map-desc').value, 10)
      : -1;
    ...
    try {
      ...
      if (
        ...
        if (ignoradosExemplos.length < 8) {
          ignoradosExemplos.push({
            linha: start + i + 1,
            motivo: 'Linha sem descrição válida',
            nome: nomeOriginal
          });
        }
        continue;
      ...
      if (
        ...
        if (ignoradosExemplos.length < 8) {
          ignoradosExemplos.push({
            linha: start + i + 1,
            motivo: 'Linha de combo/promoção ignorada',
            nome: nomeOriginal
          });
        }
        continue;
      ...
      if (precoLiq <= 0) {
        ...
        if (ignoradosExemplos.length < 8) {
          ignoradosExemplos.push({
            linha: start + i + 1,
            motivo: 'Preço líquido inválido ou zerado',
            nome: nomeOriginal
          });
        }
        continue;
      ...
      if (!prod) {
        ...
        const histIdx = prod.hist_cot.findIndex(
          (h) => h.mes === mesCotacao && h.forn === forn.nome
        );
        if (histIdx >= 0) {
  ...
  try {
    ...
    const layoutResult = await SB.toResult(() =>
      SB.upsertCotLayout({
        filial_id: State.FIL,
        fornecedor_id: forn.id,
  ...
  const configResult = await SB.toResult(() =>
    SB.upsertCotConfig({
      filial_id: State.FIL,
      locked: CCFG().locked,
      logs
  ...
  if (falhas > 0) {
    toast(
      `Importação concluída com falhas: ${novos} novos, ${atualizados} atualizados, ${falhas} erros`
    );
  } else {
