...
function resolveArg(token, el, event) {
  ...
  const flowMatch = normalized.match(
    /window\.__flowSteps\?\.([a-z]+)\s*\|\|\s*1\)\s*([+-])\s*(\d+)/
  );
  if (flowMatch) {
...
function getHandlerRegistry() {
  return {
    ...
    acessos: {
      ...
      reenviarConviteUsuarioAcesso: deps.reenviarConviteUsuarioAcesso
    },
    campanhas: {
...
function callFieldHandler(name, el, event) {
  const fieldHandlers = {
    ...
    setFiltroNotificacoes: () =>
      deps.setFiltroNotificacoes?.(
        el instanceof HTMLInputElement || el instanceof HTMLSelectElement ? el.value : undefined
      )
  };
