-- 55_fix_trigger_estoque_jsonb.sql
-- Objetivo: Corrigir a falta de baixa de estoque quando pedidos.itens contem um array jsonb stringificado.
-- Causa: O fix 53 ignorava silenciosamente strings em pedidos.itens, o que previnia o erro "cannot extract elements from a scalar",
--        mas causava a nao geracao das saidas de estoque.
-- Solução: Utilizar public.safe_pedido_itens_jsonb() para fazer parse seguro.
...
begin
  -- Pre-processa itens legados do pedido para garantir que seja um array JSONB valido
  ...
  if (TG_OP = 'INSERT' and new.status not in ('orcamento', 'cancelado')) or
    ...
    -- Fallback: coluna JSONB legada — parseada com seguranca
    if v_itens_ok = 0
